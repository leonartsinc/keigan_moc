...
// 360°他者評価（複数）：評価者列に表示する人物名
const PERSON_NAMES = ['田中 太郎', '佐藤 花子', '山田 一郎', '鈴木 美咲'];
...
// 評価者列：1つの dl の main-eval-val li の高さを対応する選択項目 li に合わせる
function syncDlHeights(dl) {
    const mainDd = dl.querySelector(':scope > dd:not(.main-eval-val)');
    const evalDds = dl.querySelectorAll(':scope > dd.main-eval-val');
    if (!mainDd || evalDds.length === 0) return;
    const mainLis = Array.from(mainDd.querySelectorAll(':scope > ul > li'));
    if (mainLis.length === 0) return;
    evalDds.forEach(evalDd => {
        const evalLis = Array.from(evalDd.querySelectorAll(':scope > ul > li'));
        mainLis.forEach((li, i) => {
            if (evalLis[i]) evalLis[i].style.height = li.offsetHeight + 'px';
        });
    });
}
...
// 評価者列：ヘッダー挿入（#person-eval-header に評価者人数分の見出しを追加）
function insertMainEvalHeaders() {
    const hdr = document.getElementById('person-eval-header');
    if (!hdr) return;
    PERSON_NAMES.forEach(name => {
        const h3 = document.createElement('h3');
        h3.className = 'main-eval-hd';
        h3.textContent = name + 'さんの評価';
        hdr.appendChild(h3);
    });
}

// 評価者列：各 dl に評価者人数分の dd（main-eval-val）を追加
// past-val は廃止したため、本文 dd の選択肢数（li 数）に合わせて空の列を新規生成する
function insertMainEvalColumns() {
    document.querySelectorAll('div.c-subsection dl').forEach(dl => {
        const mainDd = dl.querySelector(':scope > dd');
        if (!mainDd) return;
        const liCount = mainDd.querySelectorAll(':scope > ul > li').length;
        if (liCount === 0) return;

        PERSON_NAMES.forEach(() => {
            const dd = document.createElement('dd');
            dd.className = 'main-eval-val';
...
// 評価者列：今年度の選択ラジオ値を data-past にセット（評価者列全てに同値を反映するモック表示）
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 評価者列：本人の選択状態から値を補完（モック表示）
    populateMainEvalValues();
