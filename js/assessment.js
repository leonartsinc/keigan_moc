...
async function fetchAssessmentData() {
    const res = await fetch('data/assessment_data.json');
    if (!res.ok) throw new Error('assessment_data.json の読み込みに失敗しました');
    return res.json();
}
...
function initPastYearValues() {
    ...
    document.querySelectorAll('dd.past-val li[data-past]').forEach(li => {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        console.warn('assessment_data.json の適用をスキップ:', e.message);
    } finally {
        hideLoadingModal();
    }
...
// ページヘッダー（タイトル行）とメイン評価列見出し行（本人評価等の列名）を、
